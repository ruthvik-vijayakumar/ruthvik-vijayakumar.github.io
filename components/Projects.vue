...
const projects = await useAsyncData("projects", () =>
  queryContent("projects").sort({ start_date: -1 }).find(),
);
</script>
...
<template>
  <section id="projects" class="mb-16 scroll-mt-16 md:mb-24 lg:mb-36 lg:scroll-mt-24" aria-label="Selected projects">
    ...
    <div>
      <ul class="group/list">
        <ProjectItem v-for="(project, index) in projects.data.value" 
